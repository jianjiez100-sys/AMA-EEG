...
if not os.path.isdir(PROJ_TEXT_DIR) or not os.path.isdir(PROJ_IMAGE_DIR):
    print("\nProjected directories not found. Run train_text_image_alignment.py first.")
...
print(f"    Text:   {sil_t_orig:.4f} -> {sil_t_proj:.4f} (delta={sil_t_proj - sil_t_orig:+.4f})")
print(f"    Image:  {sil_i_orig:.4f} -> {sil_i_proj:.4f} (delta={sil_i_proj - sil_i_orig:+.4f})")
...
print("  Diagonal Cosine (配对 text_i/image_i 直接相似度):")
...
# ================================================================
# 图4 - Before: 按情绪类别着色 (o=Text, ^=Image)
# ================================================================
...
ax.set_title("Before Alignment - by Emotion (o=Text, ^=Image)")
ax.set_xticks([]); ax.set_yticks([]); ax.set_aspect('equal')
ax.grid(True, alpha=0.12, linestyle='--')
save_fig(fig, "04_before_by_emotion.png")

# ================================================================
# 图5 - After: 按情绪类别着色 (o=Proj Text, ^=Proj Image)
# ================================================================
fig, ax = plt.subplots(figsize=(14, 10))
for emo in EMO_ORDER:
    c = EMOTION_COLORS[emo]
...
ax.legend(handles=legend_elements, loc='center left', bbox_to_anchor=(1.02, 0.5),
          fontsize=9, framealpha=0.9, ncol=1)
ax.set_title("After Alignment - by Emotion (o=Proj Text, ^=Proj Image)")
ax.set_xticks([]); ax.set_yticks([]); ax.set_aspect('equal')
ax.grid(True, alpha=0.12, linestyle='--')
...
print("  图2: 02_after_by_modality.png  - 对齐后 Text/Image 模态分布")
print("  图3: 03_text_shift.png         - Text 位移轨迹")
print("  图4: 04_before_by_emotion.png  - 对齐前按 9 类情绪着色")
print("  图5: 05_after_by_emotion.png   - 对齐后按 9 类情绪着色")
